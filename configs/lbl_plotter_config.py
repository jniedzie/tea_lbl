...
samples = (
  Sample(
    name="lbl",
    file_path=f"{base_path}/lbl/{skim}/merged_{skim}histograms.root", 
  ...
  Sample(
    name="cep",
    file_path=f"{base_path}/cep/{skim}/merged_{skim}histograms.root",
    type=SampleType.background,
    cross_section=0.0028210336267208303,
    initial_weight_sum = 8862.0,
  ...
  Sample(
    name="qed",
    file_path=f"{base_path}/qed/{skim}/merged_{skim}histograms.root", 
    type=SampleType.background,
    cross_section=8827.220 * 0.8477 * 0.9322 * 0.952**2 * 1.0006 * 0.8643, # xsec * SFs
  ...
  Sample(
    name="data", 
    file_path=f"{base_path}/collisionData/{skim}/merged_{skim}histograms.root", 
...
#           name                  title logy    norm_type                    rebin xmin   xmax  ymin    ymax,    xlabel                ylabel            suffix
  ...
  Histogram("cutFlow"               , "", True  , NormalizationType.to_lumi, 1  ,   0   , 11   , 1e0   , 1e6   , "Selection"         , "#sum genWeight"  ),
)
